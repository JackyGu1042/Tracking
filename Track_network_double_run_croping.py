...
with tf.Session() as sess:
    ...
    for num_img in range(initail_frame,end_frame):
        ...
        if num_img == initail_frame:
            ...
            x_center =int((y_crop[0]+0.5)*width_original)
            y_center =int((y_crop[1]+0.5)*height_original)
            width = int((y_crop[2]+0.5)*width_original)
            height = int((y_crop[3]+0.5)*height_original)
        ...
        x_input_k1crop_cur = image_input_cur[y_center - int(height*k1/2):y_center + int(height*k1/2), x_center - int(width*k1/2):x_center + int(width*k1/2)]
        ...
        x_input_k1crop_pre = image_input_pre[y_center - int(height*k1/2):y_center + int(height*k1/2), x_center - int(width*k1/2):x_center + int(width*k1/2)]
...
for num_img in range(initail_frame,end_frame):
    ...
    img_rec_out = cv2.rectangle(image_test,(x_lt_crop,y_lt_crop),(x_rb_crop,y_rb_crop),(0,155,0),2)
...
skvideo.io.vwrite("drunk_video_croping_d4l.mp4", vide_output)
print "video built"
cv2.waitKey(0)
